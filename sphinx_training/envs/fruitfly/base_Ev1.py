...
class FruitflyEnv(mjx_env.MjxEnv):
    ...
    def compile(self, forced=False, mjx_model=False) -> None:
        ...
        if not self._compiled or forced:
            ...
            key_id = mujoco.mj_name2id(self._mj_model, mujoco.mjtObj.mjOBJ_KEY, 'default_pose'+self._suffix)
            if key_id >= 0:
                key_qpos = self._mj_model.key_qpos[key_id]
                for jnt_id in range(self._mj_model.njnt):
                    qa = self._mj_model.jnt_qposadr[jnt_id]
                    self._mj_model.qpos_spring[qa] = key_qpos[qa]
                print(f"Set qpos_spring from 'default_pose' keyframe for {self._mj_model.njnt} joints")
            if mjx_model:
