...
class Imitation(fruitfly_base.FruitflyEnv):
    ...
    def reset(self, rng: jax.Array, clip_idx: Optional[int] = None) -> mjx_env.State:
        ...
        info["prev_ctrl"] = jnp.zeros(self.mjx_model.nu)
    ...
    def step(
        ...
        if not self._enable_flight:
            action = action * self._zero_ctrl_mask
    ...
    def _control_cost(self, data, info, metrics, current_target, weight) -> float:
        ctrl = jnp.nan_to_num(data.ctrl, nan=0.0)
        metrics["ctrl_sqr"] = ctrl_sqr = jnp.sum(jnp.square(ctrl))
    ...
    def _control_diff_cost(self, data, info, metrics, current_target, weight) -> float:
        ctrl = jnp.nan_to_num(data.ctrl, nan=0.0)
        prev_ctrl = jnp.nan_to_num(info.get("prev_ctrl", data.ctrl), nan=0.0)
        metrics["ctrl_diff_sqr"] = ctrl_diff_sqr = jnp.sum(jnp.square(ctrl - prev_ctrl))
