...
def get_quat(theta=0, rot_axis=[0., 0, 1]):
    ...
    """
    axis = rot_axis / jnp.linalg.norm(rot_axis)
    c = jnp.cos(theta / 2)
    s = jnp.sin(theta / 2)
    return jnp.array((c, axis[0] * s, axis[1] * s, axis[2] * s))


def random_quat():
    """Returns normalized random quaternion."""
    theta = 2 * jnp.pi * jnp.random.rand()
    axis = 2 * jnp.random.rand(3) - 1
...
def mult_quat(quat1: jnp.ndarray, quat2: jnp.ndarray) -> jnp.ndarray:
    ...
    prod = jnp.empty_like(quat1) if quat1.ndim > quat2.ndim else jnp.empty_like(
        quat2)
...
def conj_quat(quat: jnp.ndarray) -> jnp.ndarray:
...
def reciprocal_quat(quat: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    return conj_quat(quat) / jnp.linalg.norm(quat, axis=-1, keepdims=True)**2
...
def rotate_vec_with_quat(vec, quat):
    ...
    if vec[..., :-1].size > quat[..., :-1].size:
        ...
        quat = jnp.tile(quat, vec.shape[:-1] + (1, ))
    vec_aug = jnp.zeros_like(quat)
...
def vec_world_to_local(world_vec, root_quat, hover_up_dir_quat=None):
    ...
    if hover_up_dir_quat is not None:
        # For flexible broadcasting, instead of jnp.tile
        hover_up_dir_quat = jnp.zeros_like(root_quat) + hover_up_dir_quat
        root_quat = mult_quat(conj_quat(hover_up_dir_quat), root_quat)

    return rotate_vec_with_quat(world_vec, root_quat)


def log_quat(quat: jnp.ndarray) -> jnp.ndarray:
    ...
    Returns:
        Array of shape (B, 4).
    """
    norm_quat = jnp.linalg.norm(quat, axis=-1, keepdims=True)
    norm_v = jnp.linalg.norm(quat[..., 1:], axis=-1, keepdims=True)
    log_quat = jnp.empty_like(quat)
    log_quat[..., 0:1] = jnp.log(norm_quat)
    log_quat[..., 1:] = quat[..., 1:] / norm_v * jnp.arccos(
        quat[..., 0:1] / norm_quat)
    return log_quat


def quat_z2vec(vec: jnp.ndarray) -> jnp.ndarray:
    ...
    edge_inds = jnp.argwhere((vec[..., :2] == 0.).all(axis=-1, keepdims=False),size=vec.shape[0])
    if edge_inds.size:
        vec = vec.copy()
    ...
    vec.at[:].set(vec / jnp.linalg.norm(vec, axis=-1, keepdims=True))
    # Cross product with [0, 0, 1].
    axis = jnp.stack([-vec[..., 1], vec[..., 0],
                     jnp.zeros_like(vec[..., 0])],
                    axis=-1)
    axis.at[:].set(axis/jnp.linalg.norm(axis, axis=-1, keepdims=True))
    angle = jnp.arccos(vec[..., 2:3])
    # Compose quaternion.
    quat = jnp.zeros(vec.shape[:-1] + (4, ))
    quat = quat.at[..., 0:1].set(jnp.cos(angle / 2))
    quat = quat.at[..., 1:].set(jnp.sin(angle / 2) * axis)

    # Clean edge case placeholders, if there are any.
    for edge_ind in edge_inds:
        ind_vec = tuple(edge_ind) + (slice(2, 3), )
        ind_quat = tuple(edge_ind) + (slice(None), )
        # if jnp.where(vec[ind_vec] < 0,vec[ind_vec],0):
        # if vec[ind_vec] < 0:
        bool_val = vec[ind_vec]
        quat = jnp.where(bool_val < 0,quat,jnp.array([0., 1., 0., 0.]))
        quat = jnp.where(bool_val > 0,quat,jnp.array([1., 0., 0., 0.]))
...
def axis_angle_to_quat(axis: jnp.ndarray, angle: jnp.ndarray) -> jnp.ndarray:
    ...
    quat = jnp.zeros(axis.shape[:-1] + (4, ))
    quat = quat.at[..., 0].set(jnp.cos(angle / 2))
    quat = quat.at[..., 1:].set(jnp.sin(angle / 2)[..., None] * axis)
    return quat


def quat_dist_short_arc(quat1: jnp.ndarray, quat2: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    quat1 = quat1 / jnp.linalg.norm(quat1, axis=-1, keepdims=True)
    quat2 = quat2 / jnp.linalg.norm(quat2, axis=-1, keepdims=True)
    x = 2 * jnp.sum(quat1 * quat2, axis=-1)**2 - 1
    x = jnp.minimum(1., x)
    return jnp.arccos(x)


def joint_orientation_quat(xaxis: jnp.ndarray, qpos: float) -> jnp.ndarray:
...
def quat_to_angvel(quat, dt=1.):
    ...
    Returns:
        Angular velocity vector, (B, 3), in the same (global) reference frame
            as the input quat.
    """
    sin_a_2 = jnp.linalg.norm(quat[..., 1:], axis=-1, keepdims=True)
    axis = quat[..., 1:] / sin_a_2  # Normalize.
    speed = 2 * jnp.arctan2(sin_a_2, quat[..., 0:1])
    # When axis-angle is larger than pi, rotation is in opposite direction.
    if speed.shape:
        speed[speed > jnp.pi] -= 2 * jnp.pi  # speed is vector.
    elif speed > jnp.pi:
        speed -= 2 * jnp.pi  # speed is scalar.
